...
{   
    for(int i = 0 ; i < k ; i++)
    {
        ...
        while ( status[index] && comp)
        {
           if( !atomic_compare_exchange_strong(&locking,&flag2,true))
           {
               flag2 = false;
           }
           else
...
void cas_bounded(double t1 , double t2)
{
    status = new bool[n]; //dynamically allocating memory
...
int main()
{
    output = fopen("cas_bounded_output.txt","w"); //output file opening
    FILE* file_ptr; 
    double l1, l2;
